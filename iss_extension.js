...
(function(ext) {
  ...
  function updateISSPlacename() {
    $.ajax({
      type: "GET",
      dataType: "json",
      // get the name of the place or ocean below the ISS
      url: "http://issabove.pythonanywhere.com/cdata",
      success: function(data) {
        issPlacename = data.iss_location;
      },
      error: function(jqxhr, textStatus, error) {
        console.log("Error downloading ISS Placename data from ISS-Above");
      }
    });
  }
  ...
  updateISSLocation();
  updateISSPlacename();
  var poller = setInterval(updateISSLocation, 4000);
  var poller2 = setInterval(updateISSPlacename, 15000);
